...
cp user.service /etc/systemd/system/user.service &>>${vinith}
...
curl -L -o /tmp/user.zip https://roboshop-artifacts.s3.amazonaws.com/user.zip &>>${vinith}
echo -e "\e[32m<<<<<<cdapp >>>>\e[0m"
cd /app &>>${vinith}
echo -e "\e[32m<<<<<<unzip >>>>\e[0m"
unzip /tmp/user.zip &>>${vinith}
echo -e "\e[32m<<<<<<cdapp >>>>\e[0m"
cd /app &>>${vinith}
...
mongo --host mongodb.akhildevops.online </app/schema/user.js &>>${vinith}
echo -e "\e[32m<<<<<< reload restart >>>>\e[0m"
systemctl daemon-reload &>>${vinith}
systemctl enable user &>>${vinith}
systemctl restart user &>>${vinith}
